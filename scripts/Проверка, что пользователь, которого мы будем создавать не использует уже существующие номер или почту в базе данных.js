    ...
    for (user of jsonData){
        if (user.phoneNumber == testUser.phoneNumber){
            ...
            phoneValidation = false
             ...
             pm.expect(phoneValidation, "Этот пользователь будет использовать  уже зарезервированные в системе данные, обновите {phoneNumber} пользователя и повторите попытку").to.be.true;  
...
pm.test("Проверка на уникальный email ", function () {
    ...
    for (user of jsonData){
        if (user.emailAddress == testUser.emailAddress){
            ...
            pm.environment.set('Testing', false)
            pm.expect(emailValidation, "Этот пользователь будет использовать  уже зарезервированные в системе данные, обновите {emailAddress} пользователя и повторите попытку").to.be.true;
